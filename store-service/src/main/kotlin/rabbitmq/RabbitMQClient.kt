...
object RabbitMQClient {
    ...
    private val LOGGER = KtorSimpleLogger("ru.vgtu.storeservice.rabbitmq.RabbitMQClient")

    const val ROUTING_KEY_STATUS_ORDER: String = "status_order"
    ...
    fun init(config: ApplicationConfig) {
        ...
        channel.queueBind(config.property("rabbitmq.queue").getString(), config.property("rabbitmq.exchange").getString(),
            "${System.getenv("STORE_NAME")}_${ROUTING_KEY_STORE_ORDER}")

        isInitialized = true
    }

    fun addConsumer(consumer: DeliveryConsumer) {
