...
object RabbitMQClient {
    ...
    private val LOGGER = KtorSimpleLogger("ru.vgtu.orderservice.rabbitmq.RabbitMQClient")
    ...
    fun init(config: ApplicationConfig) {
        ...
        channel.exchangeDeclare(config.property("rabbitmq.exchange").getString(), "direct")
        channel.queueDeclare(config.property("rabbitmq.queue").getString(), true, false, false, null)
        channel.queueBind(config.property("rabbitmq.queue").getString(), config.property("rabbitmq.exchange").getString(), ROUTING_KEY_PAYMENT_ORDER)
        isInitialized = true
    }
